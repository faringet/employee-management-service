...
func MakeGetCommunicationTemplateByIDWithEntityHandler(s templateservice.Service, logger *zap.Logger) http.Handler {
	...
	h := kithttp.NewServer(
		endpoints.MakeGetCommunicationTemplateByIDWithEntityEndpoint(s),
		getCommunicationTemplateByIDWithEntityDecodeRequest,
		encodeResponse,
		opts...,
	)
...
func getCommunicationTemplateByIDWithEntityDecodeRequest(_ context.Context, r *http.Request) (interface{}, error) {
	vars := mux.Vars(r)
	idStr, ok := vars["id"]
	...
	return endpoints.GetCommunicationTemplateByIDWithEntity{ID: id}, nil
}
